...
async def run():
    ...
    # Set parameters
    await drone.param.set_float_param("MIS_TAKEOFF_ALT", 3.0)  # set takeoff height to 1 meter
    ...
    print("-- Go up 0.5 m")
    await drone.offboard.set_position_ned(PositionNedYaw(0.0, 0.0, -1, 0.0))
    await asyncio.sleep(1)

    print("-- Go 3m North, 0m East, -1m Down (actually 0.5m high from ground) within local coordinate system, turn to face East")
    await drone.offboard.set_position_ned(PositionNedYaw(3.0, 0.0, -1.0, 0.0))
    await asyncio.sleep(1)
    ...
    await drone.offboard.set_position_ned(PositionNedYaw(-1.0, 0.0, -1.0, 0.0))
    await asyncio.sleep(1)

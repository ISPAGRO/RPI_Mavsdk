...
#include <math.h>
/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes #include <usbd_cdc_if.h> #include <usb_device.h> */
...
int main(void) {
	...
	while (1) {
		...
		if (ADC_FLAG == TRUE) {
			...
			for(int j=0;j<2;j++){
			final_temp[j]= Thermistor(adcBuf[j]);
//            temp[j]= adcBuf[j]*0.04;
//            temp[j]=100-temp[j];
//            if(temp[j]<0)
//            {
//            	temp[j]=temp[j]* -1;
//            }
			...
			sprintf(msgbuf, "%d: ADC1=%d,  vicor1=%d,vicor1_1=%f,ADC2=%d,vicor2=%d,vicor2_2=%f \r\n", i, (int) adcBuf[0],
					(float)final_temp[0],(int)adcBuf[1],(float)final_temp[1]);
			HAL_UART_Transmit(&huart1, msgbuf, strlen(msgbuf), HAL_MAX_DELAY);
//            CDC_Transmit_FS((uint32_t *)msgbuf,strlen(msgbuf));
			LED_OFF();
